 ...
 *
 * Based on the Ordnance Survey "A Guide to coordinate systems in Great Britain"
 * (document C) and the Helmert transformation from OSGB36 to WGS84.
 *
 * Inputs:  easting  (metres, may include decimal metres)
 *          northing (metres, may include decimal metres)
 * Outputs: { lat, lon } in decimal degrees WGS84
 */
...
/** Convert OSGB36 easting/northing → OSGB36 lat/lon (radians) */
function osgb36ToLatLon(E: number, N: number): { phi: number; lam: number } {
  ...
  const rho =
    (a * F0 * (1 - e2)) / Math.pow(1 - e2 * sinPhi * sinPhi, 1.5);
  const eta2 = nu / rho - 1;
  ...
  const X = 1 / (cosPhi * nu);
  const XI =
    (1 / (cosPhi * 6 * Math.pow(nu, 3))) * (nu / rho + 2 * tanPhi * tanPhi);
  const XII =
    (1 / (cosPhi * 120 * Math.pow(nu, 5))) *
    (5 + 28 * tanPhi * tanPhi + 24 * Math.pow(tanPhi, 4));
  const XIIA =
    (1 / (cosPhi * 5040 * Math.pow(nu, 7))) *
    (61 + 662 * tanPhi * tanPhi + 1320 * Math.pow(tanPhi, 4) + 720 * Math.pow(tanPhi, 6));
...
/** Full OSGB36 easting/northing → WGS84 lat/lon */
export function bngToLatLon(
  easting: number,
  northing: number
): { lat: number; lon: number } {
  ...
  const xA = (nuA + h) * cosPhi * Math.cos(lam);
  const yA = (nuA + h) * cosPhi * Math.sin(lam);
  const zA = (nuA * (1 - e2) + h) * sinPhi;
...
/** Format decimal degrees to deg° min' sec" */
